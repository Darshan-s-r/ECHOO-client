...
//             if(user?.following.includes(profileUser?.email ? profileUser.email : ' ')){
//               setIsFollowing(true);
//             }
...
//  console.log('profile uer from middlepart', profileUser);
...
//   async function handleFollow(event: React.MouseEvent<HTMLButtonElement, MouseEvent>): Promise<void> {
//     try{
//         const token = localStorage.getItem("twitter_cloan_token");
//         if(!token){
//           throw new Error("you need to login to proced")
//         }
//         const responce = await axios.post("http://localhost:8080/follow",
...
//           } 
//     });
//     const following = responce.data.following; 
//     const followers = responce.data.followers; 
//     setUser({
//       ...user,
//       following: following,
//     });
...
//     followers:followers,
//   });
//     console.log('responce from /follow');
...
//       {
//         title: "Posts",
//           link: `http://localhost:3000/${profileUser?.email}`,
//       },
//       {
//         title: "Replies",
//           link: `http://localhost:3000/${profileUser?.email}`,
...
//       isFollowing ? <button onClick={handleUnfollow} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className={`border border-custom-profile-bg font-bold text-xl px-4 py-2 rounded-3xl ${isHovered ? 'bg-blackish-red' : ''}`}>Following</button> : 
...
//     <Link className='hover:underline' href={`http://localhost:3000/${profileUser?.email}/following`}><span className='ml-4 text-lg text-custom-white font-bold'>{profileUser?.following.length}</span><span className='text-lg text-custom-grey'> Following</span> </Link>
//     <Link className='hover:underline' href={`http://localhost:3000/${profileUser?.email}/followers`}><span className='ml-4 text-lg text-custom-white font-bold'>{profileUser?.followers.length}</span><span className='text-lg text-custom-grey'> Followers</span></Link>
//     </div>
//     {/* hear i have to put Followed by ....... */}
